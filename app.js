...
const fresh=()=>({
 ...
 repSeen:[0],actorsSeen:[],cyclePlaced:[],plan:[],
...
function compactModule(){
 return {v:3,b:state.badges,rs:state.repSeen,as:state.actorsSeen,cp:state.cyclePlaced,pl:state.plan,
  dni:state.docNeedIndex,dnd:state.docNeedDone,evi:state.evolIndex,evd:state.evolDone,dsp:state.dossierPiece,
  dss:state.dossierSelected,dsd:state.dossierDone,dst:state.dossierTry,se:state.sections,ci:state.calcStep,
...
function renderRepere(n){n=+n;state.repSeen=Array.from(new Set([...(state.repSeen||[]),n]));qa('.repere-select').forEach(b=>b.classList.toggle('selected',+b.dataset.repere===n));html($('repere-stage'),REP_CONTENT[n]);text($('repere-counter'),`${state.repSeen.length}/4 consultés`);$('repere-next').disabled=state.repSeen.length<4;if(state.repSeen.length===4)complete('bud-m1-reperes');save()}
...
/* Plan */
function initPlan(){qa('#plan-choices .choice').forEach(b=>{b.classList.toggle('selected',state.plan.includes(+b.dataset.plan));b.onclick=()=>{toggleLimited(b,3);state.plan=qa('#plan-choices .choice.selected').map(x=>+x.dataset.plan);text($('plan-counter'),`${state.plan.length}/3 sélectionnées`);$('plan-save').disabled=state.plan.length!==3;save()}});text($('plan-counter'),`${state.plan.length}/3 sélectionnées`);$('plan-save').disabled=state.plan.length!==3}
function savePlan(){if(state.plan.length!==3)return;let fb=$('plan-feedback');if(!fb){fb=document.createElement('div');fb.id='plan-feedback';$('plan-choices').insertAdjacentElement('afterend',fb)}if(state.plan.includes(3)){qa('#plan-choices .choice').forEach(b=>b.classList.toggle('wrong',+b.dataset.plan===3&&b.classList.contains('selected')));feedback('plan-feedback','bad','<strong>À ajuster.</strong> Lire chaque ligne dans l’ordre sans prioriser risque de consommer votre temps sans faire ressortir les enjeux. Remplacez cette action par une démarche de repérage, de vérification ou de préparation des questions.');return}feedback('plan-feedback','ok','<strong>Plan enregistré.</strong> Vos trois premières actions rejoignent votre carnet de mission.');complete('bud-m1-plan');renderCarnet();$('plan-save').textContent='Voir le point d’étape';$('plan-save').onclick=()=>go('m1-close');save()}
...
function renderNeed(){ensureOrders();const i=state.docNeedIndex||0,row=D.DOC_NEEDS[i];text($('doc-need-counter'),`Besoin ${i+1} sur 4`);text($('doc-need-title'),row[0]);clearFeedback('doc-feedback');const box=$('doc-options');box.innerHTML='';state.docNeedOrder[i].forEach(idx=>{const lab=idx===0?row[1]:row[2],b=document.createElement('button');b.className='choice';b.textContent=lab;b.onclick=()=>{qa('.choice',box).forEach(x=>x.classList.remove('selected','correct','wrong'));b.classList.add(idx===0?'correct':'wrong');if(idx===0){state.docNeedDone[i]=true;feedback('doc-feedback','ok',`<strong>Oui.</strong> ${esc(row[4])}`)}else feedback('doc-feedback','bad',`<strong>Pas pour ce besoin.</strong> ${esc(row[4])} Vous pouvez modifier votre choix.`);save();renderNeedButtons()};box.appendChild(b)});renderNeedButtons()}
function renderNeedButtons(){const i=state.docNeedIndex;$('doc-prev').disabled=i===0;$('doc-nextneed').disabled=i===3;$('doc-screen-next').disabled=!state.docNeedDone.every(Boolean);if(state.docNeedDone.every(Boolean))complete('bud-m2-documents')}
...
function renderEvol(){ensureOrders();const i=state.evolIndex||0,row=D.EVOL_CASES[i];text($('evol-counter'),`Cas ${i+1} sur 4`);text($('evol-title'),row[0]);clearFeedback('evol-feedback');const box=$('evol-options');box.innerHTML='';state.evolOrder[i].forEach(idx=>{const lab=idx===0?row[1]:row[2],b=document.createElement('button');b.className='choice';b.textContent=lab;b.onclick=()=>{qa('.choice',box).forEach(x=>x.classList.remove('correct','wrong'));if(idx===0){b.classList.add('correct');state.evolDone[i]=true;feedback('evol-feedback','ok',`<strong>Oui.</strong> « ${esc(row[1])} » correspond à ce besoin dans le cycle budgétaire.`);$('evol-next').disabled=false}else{b.classList.add('wrong');feedback('evol-feedback','bad',`<strong>À revoir.</strong> Ce document répond à une autre fonction. Pour ce besoin, cherchez le document qui sert à « ${esc(row[0].toLowerCase())} ».`)}save()};box.appendChild(b)});$('evol-next').disabled=!state.evolDone[i]}
...
function checkHypotheses(){if(isComplete('bud-m4-trajectoire')){go('bud-m4-simulateur');return}const bad=state.hypotheses.some(i=>$('hypo-choices').querySelector(`[data-hypo="${i}"]`).dataset.ok!=='1');if(bad){qa('#hypo-choices .choice.selected').forEach(b=>b.classList.toggle('wrong',b.dataset.ok!=='1'));feedback('hypo-feedback','bad','<strong>Une hypothèse conclut trop vite.</strong> La baisse du nouvel emprunt, prise isolément, ne suffit pas à démontrer une amélioration globale : rapprochez-la de l’épargne, de la dette et de la trajectoire.');return}feedback('hypo-feedback','ok','<strong>Hypothèses solides.</strong> Vous avez distingué l’événement ponctuel des facteurs qui restent à instruire.');complete('bud-m4-trajectoire');$('hypo-check').textContent='Continuer vers le comparateur';save()}
